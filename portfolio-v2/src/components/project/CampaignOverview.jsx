...
const STAGES = [
    ...
    tactics: [
      {
        ...
        items: ["Influencer Marketing", "PR Boxes", "Organic Social Media"],
      },
  ...
  },
  {
    ...
    tactics: [
      {
        group: "Website",
        items: ["Interactive Microsite", "City Hub", "Player Cards"],
      },
      ...
      <div className="container">
        ...
        <div className="co-metrics">
          {STAGES.map((s) => (
            ...
            >
              <span className="co-metric__value" style={{ color: s.color }}>
                {s.metric}
              </span>
                ...
                <div className="co-tactics__inner">
                  {s.tactics.map((t, ti) => (
                    <div key={ti} className="co-tactic-group">
                      <div
                        className="co-tactic-group__dot"
  ...
  return (
    <div className="co-schedule">
      <div className="co-schedule__years">
        ...
        <div className="co-schedule__year">
          2026 <span>(8 months)</span>
        </div>
        <div className="co-schedule__year">
          2027 <span>(10 months)</span>
        </div>
      </div>
      ...
      <div className="co-schedule__months">
        <div className="co-schedule__label-col" />
        {MONTHS.map((m, i) => (
          <div key={i} className="co-schedule__month">
            {m}
          </div>
        ))}
      </div>
      ...
      {STAGES.map((stage) => (
        <React.Fragment key={stage.id}>
          <div
            className="co-schedule__stage-row"
          ...
          >
            <div className="co-schedule__label-col co-schedule__stage-label">
              <img
                src={stage.logo}
                alt={stage.tag}
                className="co-schedule__stage-logo"
              />
            </div>
          ...
          {stage.schedule.map((row, ri) => (
            <div
              key={ri}
              className={`co-schedule__tactic-row ${
                hoveredRow === `${stage.id}-${ri}` ? "hovered" : ""
              }`}
              onMouseEnter={() => setHoveredRow(`${stage.id}-${ri}`)}
              onMouseLeave={() => setHoveredRow(null)}
            >
              <div className="co-schedule__label-col co-schedule__tactic-label">
                {row.label}
              </div>
                ...
                {Array.from({ length: TOTAL }, (_, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={
                        active ? { background: stage.color, opacity: 0.75 } : {}
                      }
                    />
                  );
                })}
  ...
  return (
    ...
    <div className="campaign-overview">
        ...
        <div className="co-tabs">
          <button
            className={`co-tab ${tab === "overview" ? "active" : ""}`}
            onClick={() => setTab("overview")}
          >
            Overview
          </button>
          <button
            className={`co-tab ${tab === "schedule" ? "active" : ""}`}
